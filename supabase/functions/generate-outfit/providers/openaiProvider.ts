/**
 * OpenAIProvider for generate-outfit — text-only reasoning over the wardrobe
 * context (no vision needed here, unlike analyze-garment). Model name is
 * read from OPENAI_TEXT_MODEL so it can be swapped without touching code;
 * defaults to "gpt-5-mini", which handles structured JSON reasoning tasks
