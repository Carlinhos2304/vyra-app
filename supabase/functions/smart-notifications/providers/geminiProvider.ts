/**
 * GeminiProvider for smart-notifications — default provider (free tier),
 * same shape as daily-suggestion's/generate-outfit's. Pure text/JSON
 * reasoning, no image step.
 */

import { AIProvider, AIProviderError, SmartNotificationContext, SmartNotificationResult } from './types.ts';
import { buildSystemPrompt, buildUserPrompt, normalizeSmartNotifications } from './normalize.ts';
...
export class GeminiProvider implements AIProvider {
  ...
  async generateSmartNotifications(context: SmartNotificationContext): Promise<SmartNotificationResult> {
    ...
    try {
      ...
      return normalizeSmartNotifications(parsed);
    } catch (err) {
