...
export default function SelectOutfitScreen() {
  ...
  const fetchUserOutfits = async () => {
    try {
      ...
      setError(err.message || 'Failed processing your lookbook assets.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <PremiumScreen>
      <FlatList
        ...
        ListHeaderComponent={<View style={styles.headerBlock}><SectionHeader title="Select Outfit" subtitle={eventId ? "Anchor lookup style to event workspace" : `Assign style layout context for ${date}`} /></View>}
        ListEmptyComponent={isLoading ? <ActivityIndicator size="small" color="#1C1917" style={{ marginTop: 40 }} /> : <Text style={styles.emptyLabel}>No available looks.</Text>}
        renderItem={({ item }) => (
          <PremiumCard style={styles.card} onPress={() => !isSaving && handleSelectOutfit(item.id)}>
            <View style={styles.imageWrapper}>
              {item.coverImage ? <Image source={{ uri: item.coverImage }} style={styles.cardImage} /> : <View style={styles.placeholderContainer}><MaterialCommunityIcons name="hanger" size={24} color="#A8A29E" /></View>}
            </View>
            <View style={styles.cardInfo}>
              <SectionTitle numberOfLines={1} style={styles.outfitTitle}>{item.name}</SectionTitle>
              <Text style={styles.garmentCountText}>{item.garmentCount} items</Text>
            </View>
          </PremiumCard>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  card: { width: CARD_WIDTH, backgroundColor: '#FFFFFF', borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: '#F5F5F4', padding: 0 },
  imageWrapper: { width: '100%', height: CARD_WIDTH * 1.3, backgroundColor: '#F5F5F4' },
  cardImage: { width: '100%', height: '100%', resizeMode: 'cover' },
  placeholderContainer: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  cardInfo: { padding: 10 },
  outfitTitle: { color: '#1C1917', fontSize: 13, fontWeight: '600' },
  garmentCountText: { fontSize: 12, color: '#78716C', marginTop: 2 },
  emptyLabel: { textAlign: 'center', color: '#78716C', marginTop: 40, fontSize: 13 },
