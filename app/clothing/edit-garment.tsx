...
export default function EditGarmentScreen() {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('beforeRemove', (e) => {
      ...
      Alert.alert(
        'Discard Changes?',
        'You have edited parameters on this garment structure. Leaving now drops modifications permanently.',
        [
  ...
  const pickImageFromGallery = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission Denied', 'Vyra needs access to your camera roll to adjust photos.');
      return;
    }
  ...
  const capturePhotoFromCamera = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission Denied', 'Vyra requires camera device access to capture items.');
      return;
    }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (authError || !user) {
        Alert.alert('Session Expired', 'Please authenticate again to make edits.');
        return;
      }
      ...
      <SafeAreaView style={styles.safeContainer} edges={['top']}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollBodyContainer}>
          ...
          <View style={styles.textInputWrapperBox}>
            <TextInput 
              placeholder="e.g., Cashmere Knit Mockneck"
            ...
            {CREATION_CATEGORIES.map((category) => {
              ...
              return (
                <PremiumTouchable
                  ...
                  style={[styles.categoricalChip, isSelected ? styles.chipSelected : styles.chipUnselected]}
                  disabled={isSaving}
                >
                  <Text style={[styles.chipTextLabel, isSelected ? styles.chipTextSelected : styles.chipTextUnselected]}>
                    {category}
                  </Text>
                </PremiumTouchable>
              );
            ...
            {PALETTE_COLORS.map((colorItem) => {
              ...
              return (
                <PremiumTouchable
                  ...
                  style={[styles.swatchCircleCircle, { backgroundColor: colorItem.hex }, isLightVariant && styles.whiteSwatchBorderOverride, isSelected && styles.swatchCircleActiveOutline]}
                  disabled={isSaving}
                >
          ...
          >
            {isSaving ? (
              ...
              <Text style={styles.saveExecutionText}>Save Changes</Text>
            )}
          </PremiumTouchable>
        </ScrollView>
      </SafeAreaView>
          ...
          <SafeAreaView style={styles.modalSafeBoundary} edges={['bottom']}>
            <View style={styles.bottomSheetFrame}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalScrollBody} bounces={false}>
                ...
                <View style={styles.canvasContainerFrame} onLayout={(e) => { containerWidthRef.current = e.nativeEvent.layout.width; }} {...saturationBrightnessResponder.panHandlers}>
              ...
              <View style={styles.modalActionButtonsRow}>
                ...
                <PremiumTouchable onPress={handleCustomColorConfirmation} style={styles.modalPrimaryButton}>
...
const styles = StyleSheet.create({
  ...
  textInputWrapperBox: { backgroundColor: '#F5F5F4', borderRadius: 12, height: 48, paddingHorizontal: 14, justifyContent: 'center', marginBottom: 4 },
  formInputCore: { fontSize: 14, color: '#1C1917', width: '100%' },
